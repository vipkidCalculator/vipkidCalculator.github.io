...
var elementAfterSubmitValidate = document.getElementById("after-submit-validate");
...
document.getElementById('our-form').addEventListener('submit', function(event){
	...
	element.style.color = "black";
	var shortNoticePlusTrialClasses = parseInt(elementTrialNoShowClasses.value) + parseInt(elementShortNoticeClasses.value);
	submitPermitted.submitSix = false;
	
	if(shortNoticePlusTrialClasses > parseInt(elementThisMonthClasses.value)){
		element.style.color = "red";
		elementAfterSubmitValidate.textContent = "Your number of trial no show classes + the number of short notice classes cannot be greater than the total classes this month. Please re-enter your short notice and trial classes.";
	}else{
		elementAfterSubmitValidate.textContent = "Submitted! :)";
